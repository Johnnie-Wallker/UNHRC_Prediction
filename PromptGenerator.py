...
def prompt_generator(data, education, work, task_id, stage, train, summary):
    description = ''
    # 对语言能力进行替换
    ...
    # 计算入选人数
    if stage == 1:
        task_data.loc[:, 'count'] = task_data['interviewed'].sum()
    if stage == 2:
        task_data['count'] = 1
        data = data.drop(columns=['interviewed'])
        data = data.rename(columns={'selected': 'interviewed'})
    if train and task_id in task_id_map.loc[:, 'task_id'].values:
        information = ''
